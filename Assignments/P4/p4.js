...
// buttons to change viewing parameters
    while (true) {
        ...
        y = points.y[t];
        z = points.z[t];
        //allows for circular  rotation and up and down camera movement
        if (phiClicked || thetaClicked) {
            x = radius*Math.sin(phi)*100 + 8;
            y = radius*Math.sin(theta)*100 + 5;
            z = radius*Math.cos(phi)*100 + 42;
            at = vec3(8, 5, 42);
        }
        eye = vec3(x, y, z);
        if (stepping) { t++; }
